...
trait DatatablTrait {
    ...
    public function status($isYes, $id , $url , $item=NULL) {
        ...
        if( ($isYes=='yes' || $isYes=='YES' || $isYes=='Yes' ) && $isYes !== NULL ){
            $isYes = '<div class="custom-control custom-switch custom-switch-on-success">
                <input id="status_'.$id.'" name="status_'.$id.'" data-url="'.$url.'" type="checkbox" class="change-status custom-control-input"  value="'.$id.'"  checked />
                <label for="status_'.$id.'" class="custom-control-label"></label>
            </div>';
        }else{
            $isYes = '<div class="custom-control custom-switch custom-switch-on-success">
                <input id="status_'.$id.'" name="status_'.$id.'" class="change-status custom-control-input" data-url="'.$url.'" type="checkbox"  value="'.$id.'"   />
                <label for="status_'.$id.'" class="custom-control-label"></label>
            </div>' ;
        }
